...
void process(int x,int y,bool dir); // sterge liniile care sunt 3 in a row
...
void afisare (){
    ...
    for(i=1; i<=n; i++)
    {
        ...
        {
            if(t[i][j]==0)     cout<<"  ";
            else if(t[i][j]==1){SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),14);cout<<(char) 233<<' ';}
            else if(t[i][j]==2){SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),6);cout<<(char) 254<<' ';}
            else if(t[i][j]==3){SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),4);cout<<(char) 234<<' ';}
            else if(t[i][j]==4){SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),9);cout<<(char) 167<<' ';}
        }SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),15);
        cout<<'|'<<'\n';
    }
}
...
void process(int x,int y,bool dir){
    ...
    if(dir==0){k=1;
        while (t[x][y]==t[x+k][y] && x+k<=n)
            {t[x+k][y]=0;s++;k++;afisare();}k=1;
        while (t[x][y]==t[x-k][y] && x-k>=1)
        {t[x-k][y]=0;s++;k++;afisare();}
        }
    else {
        k=1;
        while (t[x][y]==t[x][y+k] && y+k<=n)
            {t[x][y+k]=0;s++;k++;afisare();}k=1;
        while (t[x][y]==t[x][y-k] && y-k>=1)
            {t[x][y-k]=0;s++;k++;afisare();}
    }
    stemp+=(s+1);
}
...
void refill(bool fa) {
    ...
    for(j=1;j<=n;j++)
    for(i=n;i>=1;i--) {k=i;
        while (t[k][j]!=0 && t[k+1][j]==0 && k<9) {swap(t[k+1][j],t[k][j]);k++;if(fa)afisare();}
        while (t[1][j]==0) {
                t[1][j]=rand()%4+1; k=1;if (fa)afisare();
                while (k<9 && t[k+1][j]==0) {swap(t[k][j],t[k+1][j]);k++;if(fa)afisare();}
        }
    }
}
