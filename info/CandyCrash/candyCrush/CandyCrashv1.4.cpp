...
int n=9,t[15][15],S,stemp,miscari,mult=2,poly[10],speed; char x[300];
...
void *nik,*rock,*bomba,*coroana,*strugure,*portocala,*pepene,*pruna;
int main (){
    //read highscore.sav
...
void loadimages() {
    setactivepage(2);
    readimagefile("./skins/nica.jpg",1,1,62,62);
    nik=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,nik);
    readimagefile("./skins/mar.jpg",1,1,62,62);
    pepene=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,pepene);
    readimagefile("./skins/struguri.jpg",1,1,62,62);
    strugure=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,strugure);
    readimagefile("./skins/coro.jpg",1,1,62,62);
    coroana=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,coroana);
    readimagefile("./skins/capsuna.jpg",1,1,62,62);
    pruna=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,pruna);
    readimagefile("./skins/porto.jpg",1,1,62,62);
    portocala=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,portocala);
    readimagefile("./skins/rock.jpg",1,1,62,62);
    rock=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,rock);
    readimagefile("./skins/bomba.jpg",1,1,62,62);
    bomba=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,bomba);
    setactivepage(1);
    setvisualpage(1);
}
    ...
    for(i=1;i<=5;i++)
        if(li[i].scor<S) {
            ...
            do{
                ...
                else if(j<9 && x[200]>31 && x[200]<127) {x[++j]=x[200]; x[j+1]='\0';}
                if(j<-1) j=-1;
                outtextxy(310,600,x);
            }while(x[200]!='\r');
...
///desene
void boardinit () {
    readimagefile("./skins/bg.jpg",0,0,700,800);
...
void afisare (){
    ...
    for(i=1; i<=n; i++)
    {
        for(j=1; j<=n; j++)
            switch (t[j][i]) {
            case 0: {//nimic(i,j);
                 putimage(i*64+1,j*64+1,nik,COPY_PUT); break;}
            case 1: {//cerc(i,j);
                 putimage(i*64+1,j*64+1,pruna,COPY_PUT); break;}
            case 2: {//patrat(i,j);
                 putimage(i*64+1,j*64+1,portocala,COPY_PUT); break;}
            case 3: {//triunghi(i,j);
                 putimage(i*64+1,j*64+1,pepene,COPY_PUT); break;}
            case 4: {//gogoasa(i,j);
                 putimage(i*64+1,j*64+1,strugure,COPY_PUT); break;}
            case 5: {//corona(i,j);
                 putimage(i*64+1,j*64+1,coroana,COPY_PUT); break;}
            case 6: {//bomba(i,j);
                 putimage(i*64+1,j*64+1,bomba,COPY_PUT); break;}
            case 7: {//useless(i,j);
                 putimage(i*64+1,j*64+1,rock,COPY_PUT); break;}
            }
    }
    Sleep(speed);
...
void drawtitle() {
    ...
    readimagefile("./skins/tit.jpg",1,1,700,800);
    showsc(); // scor
    setcolor(WHITE);
    //
    setfillstyle(1,BLACK);
...
void triunghi(int b,int a) {
    a=a*64; b=b*64;
    //readimagefile("./skins/mar.jpg",a+1,b+1,a+62,b+62);
}

void cerc(int b,int a) {
    a=a*64; b=b*64;
    //readimagefile("./skins/capsuna.jpg",a+1,b+1,a+62,b+62);
}

void patrat(int b,int a) {
    a=a*64; b=b*64;
    //readimagefile("./skins/porto.jpg",a+1,b+1,a+62,b+62);
}

void gogoasa(int b,int a) {
    a=a*64; b=b*64;
    //readimagefile("./skins/struguri.jpg",a+1,b+1,a+62,b+62);
    //putimage(a,b,strugure,COPY_PUT);
}

void corona(int b,int a) {
    a=a*64; b=b*64;
    //readimagefile("./skins/coro.jpg",a+1,b+1,a+62,b+62);
    //putimage(a,b,coroana,COPY_PUT);
}

void bomba(int b,int a) {
    a=a*64; b=b*64;
    //readimagefile("./skins/bomba.jpg",a+1,b+1,a+62,b+62);
    //putimage(a,b,bomba,COPY_PUT);
}

void useless(int b,int a) {
    a=a*64; b=b*64;
    //readimagefile("./skins/rock.jpg",a+1,b+1,a+62,b+62);
    //putimage(a,b,rock,COPY_PUT);
}

void nimic(int b,int a) {
    a=a*64; b=b*64;
    //readimagefile("./skins/nica.jpg",a+1,b+1,a+62,b+62);
    //putimage(a,b,nik,COPY_PUT);
}
*/
void sel(int a,int b) {
    a=a*64; b=b*64;
