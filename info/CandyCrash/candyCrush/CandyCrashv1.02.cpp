...
int main (){
    ...
    for(stemp=1;stemp<=5;stemp++) {fin>>li[stemp].scor; fin.getline(li[stemp].nume,15);}
    fin.close();
    initwindow(700,800);
    title();
    closegraph();
...
void plusbum(int i,int j) {
    int k;
    stemp+=5; t[i][j]=0;
    for(k=1;k<=n;k++) { stemp+=t[i][k]+t[j][k];if(t[i][k]==5) plusbum(i,k); if(t[k][j]==5) plusbum(k,j);t[i][k]=t[k][j]=0;}
}
...
void title() {
    ...
    while(1)
        if(ismouseclick(WM_LBUTTONDOWN)) { //daca dau click
            ...
            if(y1>190 && y1<270 && x1>250 && x1<450) {joc(); if(miscari==-2) return; else {drawtitle(); highscore();}} //buton start
