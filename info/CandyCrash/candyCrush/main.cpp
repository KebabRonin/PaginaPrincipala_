...
#include "graphics.h"
using namespace std;
//n = dimensiunea matricii; t = matrice; S = scor total; x = string pentru afisare;
int n=9,t[11][11],S,miscari,speed=150, vertica[11][11],orizont[11][11]; char x[300],skinname[300];
void *el[15];
struct hs{int scor; char nume[15];}li[7];
///scene
...
int main (){
    ...
    title();
    closegraph();
    ofstream fout("highscore.sav");for(int i=1;i<=5;i++) fout<<li[i].scor<<" "<<li[i].nume<<'\n'; fout<<skinname; fout.close();
    return 0;
}
...
void tutorial() {
    ...
    strcpy(x,"REGULILE JOCULUI"); outtextxy(290,102,x);
    ...
    strcpy(x,"3x3 in jurul sau, lasand in urma un element destructibil doar de un alt"); outtextxy(200,342,x);
    strcpy(x,"element de acelasi tip. Elementele sterse astfel nu cresc scorul"); outtextxy(200,362,x);
    putimage(2*64+1,7*64+1,el[7],COPY_PUT);
    strcpy(x,"Acest element nu poate fi mutat si poate fi distrus doar de elementul de mai"); outtextxy(200,468,x);
    strcpy(x,"sus"); outtextxy(200,488,x);
    //
    ...
    strcpy(x,"Pentru a selecta elemente ele trebuie apasate cu mouse-ul doar cand apare mesajul in partea"); outtextxy(60,570,x);
    strcpy(x,"de jos a ecranului. Pentru a anula selectia dati click in afara tablei de joc."); outtextxy(60,590,x);
...
void title() {
    ...
    while(1)
        if(ismouseclick(WM_LBUTTONDOWN)) { //daca dau click
            ...
            else if(buton(650,700,0,50,x1,y1)) {changeskin(); clearmouseclick(WM_LBUTTONDOWN); drawtitle();} //buton tutorial
...
void testspecial(int i,int j,int &Sl,int &Sv) {
    ...
    for(int l=1;l<5;l++) {
        ...
        if(V>=2) {k=1;
            while(l==t[i+k][j] && i+k<=n){vertica[i+k][j]=l;k++;}k=1;
            while(l==t[i-k][j] && i-k>=1){vertica[i-k][j]=l;k++;}
        }
        k=1;
        while(l==t[i][j+k] && j+k<=n){L++;k++;}k=1;
        while(l==t[i][j-k] && j-k>=1){L++;k++;}
        if(L>=2) {k=1;
            while(l==t[i][j+k] && j+k<=n){orizont[i][j+k]=l; k++;}k=1;
            while(l==t[i][j-k] && j-k>=1){orizont[i][j-k]=l;k++;}
        }
        ...
        if(t[i][j]!=0 && t[i][j]!=7) {
            if(t[i][j]==5) testspecial(i,j,k,l);
                ...
                if(!orizont[i][j]){
                    ...
                    if(l>=2) {nr=1; for(l;l>=0;l--) {orizont[i+l][j]=k; ori(i+l,j);}}
                    }
                if(!vertica[i][j]) {
                    l=0; do {l++;}while((t[i][j+l]==k || t[i][j+l]==5) && j+l<10); l--;
                    if(l>=2) {nr=1; for(l;l>=0;l--) {vertica[i][j+l]=k; ver(i,j+l);}}
                }
            }
        }
    }
    for(i=1; i<=n; i++) for(j=1; j<=n; j++) if(vertica[i][j] || orizont[i][j]) {destroy(i,j); if(vertica[i][j]==orizont[i][j] && orizont[i][j]!=0) t[i][j]=5;}
...
void changeskin() {
    ...
    // Able to open directory stream
    if (dir) {
        while ((dp = readdir(dir)) != NULL) if(strcmp(dp->d_name,skinname)==0) break;
        dp=readdir(dir);
        if(dp==NULL) {
            closedir(dir);
            dir = opendir("skins");
            dp = readdir(dir); dp = readdir(dir);// citeste primul director, sare peste . si ..
            while((dp = readdir(dir))!=NULL)
                if(strcmp(dp->d_name,skinname)!=0 && dp->d_type==16) break;
        }
        if(dp) strcpy(skinname,dp->d_name);
        else strcpy(skinname,"none");
        loadimages();
        drawtitle();
    }
    closedir(dir);
...
void loadimages() {
    int sag_st[]={4,25,25,4,25,46,4,25},coroana[]={4,4,20,16,36,4,52,16,60,4,54,60,16,60,4,4},sag[]={25,25,4,4,4,46,25,25},bolo[]={4,30,16,4,47,10,54,45,14,50,4,30};
    struct dirent *dp;
    DIR *dir = opendir("skins");
    // Able to open directory stream
    if (dir) {
        ...
        if(dp==NULL) {
            closedir(dir);
            dir = opendir("skins"); dp=readdir(dir); dp=readdir(dir);
            while((dp = readdir(dir))!=NULL) if(dp->d_type==16) {strcpy(skinname,dp->d_name); break;}
        }
    }
    closedir(dir);
    //
    char path[300];
    strcpy(path,"./skins/"); strcat(path,skinname); int l=strlen(path);
    setactivepage(2);
    //0 - nimic
    path[l]=0; strcat(path,"/nimic.jpg");
    setfillstyle(1,YELLOW); bar(0,0,700,800);
    readimagefile(path,1,1,62,62);
    el[0]=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,el[0]);
    //1 - elementul 1
    path[l]=0; strcat(path,"/e1.jpg");
    setfillstyle(1,BLACK); bar(0,0,64,64);
    readimagefile(path,1,1,62,62);
    el[1]=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,el[1]);
    //2 - elementul 2
    path[l]=0; strcat(path,"/e2.jpg");
    setfillstyle(1,DARKGRAY); bar(0,0,64,64);
    readimagefile(path,1,1,62,62);
    el[2]=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,el[2]);
    //3 - elementul 3
    path[l]=0; strcat(path,"/e3.jpg");
    setfillstyle(1,RED); bar(0,0,64,64);
    readimagefile(path,1,1,62,62);
    el[3]=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,el[3]);
    //4 - elementul 4
    path[l]=0; strcat(path,"/e4.jpg");
    setfillstyle(1,WHITE); bar(0,0,64,64);
    readimagefile(path,1,1,62,62);
    el[4]=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,el[4]);
    //5 - coroana
    path[l]=0; strcat(path,"/coroana.jpg");
    setfillstyle(1,YELLOW); bar(0,0,64,64); setcolor(BLACK); drawpoly(8,coroana); setfillstyle(1,BLUE); floodfill(32,32,BLACK);
    readimagefile(path,1,1,62,62);
    el[5]=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,el[5]);
    //6 - bomba
    path[l]=0; strcat(path,"/bomba.jpg");
    setfillstyle(1,YELLOW); bar(0,0,64,64); setcolor(BLACK); circle(32,32,30); setfillstyle(1,BLACK); floodfill(32,32,BLACK);
    readimagefile(path,1,1,62,62);
    el[6]=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,el[6]);
    //7 - piatra
    path[l]=0; strcat(path,"/bolovan.jpg");
    setfillstyle(1,YELLOW); bar(0,0,64,64); setcolor(BLACK); drawpoly(6,bolo); setfillstyle(1,BLACK); floodfill(32,32,BLACK);
    readimagefile(path,1,1,62,62);
    el[7]=malloc(imagesize(1,1,62,62));
    getimage(1,1,62,62,el[7]);
    //8 - sageata stanga
    path[l]=0; strcat(path,"/sageata_st.jpg");
    setfillstyle(1,BLACK); bar(0,0,32,52); setcolor(WHITE); drawpoly(4,sag_st); setfillstyle(1,WHITE); floodfill(16,32,WHITE);
    readimagefile(path,1,1,30,50);
    el[8]=malloc(imagesize(1,1,30,50));
    getimage(1,1,30,50,el[8]);
    //9 - sageata dreapta
    path[l]=0; strcat(path,"/sageata.jpg");
    setfillstyle(1,BLACK); bar(0,0,32,52); setcolor(WHITE); drawpoly(4,sag); setfillstyle(1,WHITE); floodfill(16,32,WHITE);
    readimagefile(path,1,1,30,50);
    el[9]=malloc(imagesize(1,1,30,50));
    getimage(1,1,30,50,el[9]);
    //10 - mainbg
    path[l]=0; strcat(path,"/title.jpg");
    setfillstyle(1,BLACK); bar(0,0,700,800);
    readimagefile(path,1,1,700,800);
    el[10]=malloc(imagesize(1,1,700,800));
    getimage(1,1,700,800,el[10]);
    //11 - jocbg
    path[l]=0; strcat(path,"/bgjoc.jpg");
    setfillstyle(1,BLACK); bar(0,0,700,800);
    readimagefile(path,1,1,700,800);
...
void drawtitle() {
    ...
    bar(650,0,700,50);
    setbkcolor(GREEN);
    strcpy(x,"schimb"); outtextxy(652,3,x);
    strcpy(x,"skin"); outtextxy(664,25,x);
    setbkcolor(BLACK);
    //
    watermark();
}
...
void watermark() {
    setbkcolor(BLACK);
    strcpy(x,"CANDY CRUSH | Skin : "); strcat(x,skinname); outtextxy(10,782,x);
